...
function slugifyHobby(name: string): string {
  return name
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "_")
    .replace(/^_+|_+$/g, "");
...
// Core ids referenced by the canned products/profiles below — keep them stable
// so mock feed items and seeded profiles still resolve to a hobby.
const CORE_HOBBIES: HobbyDto[] = [
  { id: "hobby-coffee", name: "Coffee", slug: "coffee" },
  { id: "hobby-hiking", name: "Hiking & camping", slug: "hiking_camping" },
  { id: "hobby-gaming", name: "Video gaming", slug: "video_gaming" },
  ...
  { id: "hobby-yoga", name: "Yoga & meditation", slug: "yoga_meditation" },
  { id: "hobby-music", name: "Playing music", slug: "playing_music" },
];

// Mirrored from giftgenius-engine/taxonomy/hobbies.txt so design mode matches
// production. Keep in sync with that file.
const TAXONOMY_HOBBY_NAMES: string[] = [
  "Cooking",
  "Baking",
  "Grilling & BBQ",
  "Coffee",
...
// Merge core + taxonomy, dedupe by slug (core wins so its id stays stable),
// then sort by name to match the backend's `GET /hobbies` ordering.
const MOCK_HOBBIES: HobbyDto[] = (() => {
  const bySlug = new Map<string, HobbyDto>();
  for (const hobby of CORE_HOBBIES) bySlug.set(hobby.slug, hobby);
  for (const name of TAXONOMY_HOBBY_NAMES) {
    const slug = slugifyHobby(name);
    if (!bySlug.has(slug)) {
      bySlug.set(slug, { id: `hobby-${slug}`, name, slug });
    }
  }
...
export function createMockApiClient() {
  return {
    ...
    async deleteProfile(
      ...
      return delay({ ok: true, deleted_id: profileId });
    },
